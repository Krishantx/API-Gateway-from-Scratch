...
import com.krishantx.github.com.API_Gateway.service.ValidationService;
...
public class ValidationFilter implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        */
        ...
        if (!validationService.endpointExists(route, request, response)) 
            return false;
        
        // Check if the endpoint supports that method
        if (!validationService.isMethodSupported(route, request, response)) 
            return false;
        
        
        //Check if the request contains the required headers
        if (!validationService.hasRequiredHeaders(route, request, response))
            return false;
